...
from tinygrad.runtime.support.assembler.CubinFile import CubinFile
from tinygrad.runtime.support.assembler.CuAsmParser import CuAsmParser
import argparse, io, sys, struct, dataclasses, re
from tinygrad.runtime.support.assembler.CuAsmParser import CuAsmParser, CuAsmSection, CuAsmSegment
from tinygrad.runtime.support.assembler.CuKernelAssembler import CuKernelAssembler
from tinygrad.runtime.support.assembler.CuInsAssemblerRepos import CuInsAssemblerRepos
from elftools.elf.structs import ELFStructs
from typing import Optional, Union

from tinygrad.runtime.support.assembler.CuSMVersion import CuSMVersion
...
class SASSCompiler(Compiler):
  def __init__(self, arch:str, cache_key=None):
    self.arch = arch
    super().__init__(f"compile_{cache_key}_{self.arch}")

  def compile(self, src: str):
    textBytes = encode_instruction(src)

    # elf dump section i=9
    rel_debug_frame_bytes = \
  b"\x44\x00\x00\x00\x00\x00\x00\x00\x02\x00\x00\x00\x06\x00\x00\x00"
  ...
  b'\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x80\x01\x00\x00'+\
  b'\x00\x00\x00\x00\x04\x04\x00\x00\x00\x04\x2c\x00\x00\x00\x0c\x81'+\
